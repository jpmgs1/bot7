	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SELAMAT TINGGAL... @${num.split('@')[0]}👋* \n_Kalo balik jangan lupa bawa gorengan Ngab!!_`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
			...
			switch(command) {
               ...
               client.sendMessage(from, 'Nih nomor pacarku😍, ehh ownerku hehee😁 jangan di galakin ya:*',MessageType.text, { quoted: mek} )
				...
				case 'ytmp3':
					...
					anu = await fetchJson(`https://mhankbarbar.tech/api/yta?url=${args[0]}&apiKey=${BarBar}`, {method: 'get'})
				...
				case 'tulis':
					...
					teks = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbar.tech/nulis?text=${teks}&apiKey=${BarBar}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'tagall':
					...
					break
				case 'clearall':
					if (!isOwner) return reply('𝐒𝐢𝐚𝐩𝐚 𝐊𝐚𝐮?')
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ 𝐅𝐃𝐋 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 ]\n\n${body.slice(4)}`})
						}
						reply('𝐃𝐎𝐍𝐄 𝐎𝐌')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐅𝐃𝐋 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 ]\n\n${body.slice(4)}`)
						}
						reply('𝐃𝐎𝐍𝐄 𝐎𝐌')
					}
					break
				...
				case 'admin':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('𝐑𝐞𝐩𝐥𝐲 𝐒𝐭𝐢𝐤𝐞𝐫𝐧𝐲𝐚 𝐓𝐨𝐝')
					reply(mess.wait)
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('𝐏𝐚𝐚𝐧 𝐒𝐢𝐡𝐡')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('𝐔𝐝𝐚𝐡 𝐀𝐤𝐭𝐢𝐟 𝐒𝐚𝐲...')
						welkom.push(from)
